...
class DataSource(tx.data.DataSource):
    ...
    def __init__(self, file: str, mode='test'):
        df = pd.read_csv(file, sep='\t')
        data = []
        for i in range(len(df)):
            data.append({
                'text_a': df['question1'][i],
                'text_b': df['question2'][i],
                'label': df['label'][i] if mode != 'test' else None
            })
        self.data = data
...
class Dataset(tx.data.DatasetBase):
    ...
    def __init__(self, data_path, hparams=None, device=None):
        source = DataSource(data_path, 'train')
